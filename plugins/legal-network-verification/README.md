...
### v2.0.1-beta（2026-08-13）

- **信用中国入口修复**：查询提交前确认控件未被浮层或其他链接遮挡；发现遮挡时不继续点击，也不把误入信用修复指南记为已提交。只有当次重新核验官方脚本仍指向同域结果页时，才允许执行一次带已确认条件的直接导航。
- **归档地址收敛**：实时浏览器地址可以暂时包含查询参数；schema 1.2、Markdown 和 DOCX 只保存去参数、去片段的同域规范结果页地址，查询条件继续由结构化字段留痕。
