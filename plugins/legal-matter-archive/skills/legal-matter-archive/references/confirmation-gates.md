...
1. `inspect` 只读盘点。
2. `plan` 只生成预览。来源目录内每个盘点文件都必须明确纳入或排除；未确认正式归档、委托范围、材料决定、目录、封皮、页码、小结或缺失处理时必须阻断。
...
7. 只有全部自动视觉 QA（包括封皮与最终首页一致性）通过且明确记录复核人，才能提升为 `ready_for_oa_submission`。保留既有扫描页码时，由律师在该逐页视觉复核中确认。
8. `preview-deliver` 必须绑定当前 manifest、视觉复核记录、获批 `output_root` 内的目标目录、文件名和最终 PDF 哈希；`deliver` 再次使用匹配令牌，仅复制最终 PDF，且不得写入 source roots。
